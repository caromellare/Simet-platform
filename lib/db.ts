...
export async function createUser(data: {
  ...
  const { rows } = await sql<DBUser>`
    ...
    VALUES (${data.name}, ${data.email}, ${data.passwordHash}, ${data.role ?? 'user'})
    RETURNING *
  `
  return rows[0]
}
...
export async function getUGCCreators(brand?: string) {
  if (brand) {
    const { rows } = await sql`
      SELECT * FROM ugc_creators WHERE brand = ${brand} ORDER BY created_at DESC
    `
    return rows
  }
  const { rows } = await sql`SELECT * FROM ugc_creators ORDER BY created_at DESC`
  return rows
}
...
export async function createUGCCreator(data: Record<string, unknown>) {
  const { rows } = await sql`
    INSERT INTO ugc_creators (
      ...
      ${(data.followers as number) ?? 0},
      ${data.contentType as string},
      ${(data.fee as string) ?? null},
      ${(data.freeProduct as boolean) ?? false},
      ${(data.publicationDate as string) ?? null},
      ${(data.postUrl as string) ?? null},
      ${(data.status as string) ?? 'contactado'},
      ${(data.notes as string) ?? ''},
      ${data.brand as string},
      ${JSON.stringify(data.tags ?? [])},
      ${((data.metrics as Record<string, number>) ?? {})?.views ?? null},
      ${((data.metrics as Record<string, number>) ?? {})?.likes ?? null},
      ${((data.metrics as Record<string, number>) ?? {})?.comments ?? null},
      ${((data.metrics as Record<string, number>) ?? {})?.reach ?? null},
      ${((data.metrics as Record<string, number>) ?? {})?.saves ?? null}
    )
    RETURNING *
  `
  return rows[0]
...
export async function updateUGCCreator(id: string, data: Record<string, unknown>) {
  const { rows } = await sql`
    UPDATE ugc_creators SET
      ...
      content_type = ${data.contentType as string},
      ...
      metric_views = ${((data.metrics as Record<string, number>) ?? {})?.views ?? null},
      metric_likes = ${((data.metrics as Record<string, number>) ?? {})?.likes ?? null},
      metric_comments = ${((data.metrics as Record<string, number>) ?? {})?.comments ?? null},
      metric_reach = ${((data.metrics as Record<string, number>) ?? {})?.reach ?? null},
      metric_saves = ${((data.metrics as Record<string, number>) ?? {})?.saves ?? null}
    WHERE id = ${id}
    RETURNING *
  `
  return rows[0]
...
export async function getVideos(brand?: string) {
  if (brand) {
    const { rows } = await sql`
      SELECT * FROM videos WHERE brand = ${brand} ORDER BY created_at DESC
    `
    return rows
  }
...
export async function createVideo(data: Record<string, unknown>) {
  const { rows } = await sql`
    ...
    VALUES (
      ...
      ${(data.status as string) ?? 'idea'},
      ${(data.description as string) ?? null},
      ${(data.script as string) ?? null},
      ${(data.publishDate as string) ?? null},
      ${(data.postUrl as string) ?? null},
      ${JSON.stringify(data.tags ?? [])},
      ${data.brand as string}
    )
...
export async function getContentIdeas(brand?: string) {
  if (brand) {
    const { rows } = await sql`
      SELECT * FROM content_ideas WHERE brand = ${brand} ORDER BY created_at DESC
    `
    return rows
  }
  const { rows } = await sql`SELECT * FROM content_ideas ORDER BY created_at DESC`
  return rows
}
...
export async function createContentIdea(data: Record<string, unknown>) {
  const { rows } = await sql`
    ...
    VALUES (
      ...
      ${(data.hook as string) ?? null},
      ${(data.cta as string) ?? null},
...
export async function getCampaignIdeas(brand?: string) {
  if (brand) {
    const { rows } = await sql`
      SELECT * FROM campaign_ideas WHERE brand = ${brand} ORDER BY created_at DESC
    `
    return rows
  }
  const { rows } = await sql`SELECT * FROM campaign_ideas ORDER BY created_at DESC`
  return rows
}
...
export async function getEphemeris(brand?: string) {
  if (brand) {
    const { rows } = await sql`
      SELECT * FROM ephemeris WHERE brand = ${brand} OR brand IS NULL ORDER BY date ASC
    `
    return rows
  }
  const { rows } = await sql`SELECT * FROM ephemeris ORDER BY date ASC`
  return rows
}
